...
ECIES.prototype.encrypt = function(message, ivbuf = undefined, returnStruct = false) {
  ...
  var ctbuf = c.slice(128 / 8);
  var dataToMac = Buffer.concat([ivbuf, this.Rbuf, ctbuf]);
  var d = Hash.sha256hmac(dataToMac, this.kM);
...
ECIES.prototype.decrypt = function(encbuf) {
  ...
  var ivbuf = c.slice(0, 128 / 8);
  var ctbuf = c.slice(128 / 8);

  var d = encbuf.slice(encbuf.length - tagLength, encbuf.length);

  var dataToMac = Buffer.concat([ivbuf, pub, ctbuf]);
  var d2 = Hash.sha256hmac(dataToMac, this.kM);
